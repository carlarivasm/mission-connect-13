...
const ManageLocationNotes = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const { data: notes } = await supabase
        ...
        .select("location_id, user_id, needs, notes, user_address, updated_at")
  ...
  const exportData = (format: "csv" | "xlsx") => {
    const data = rows.map((r) => ({
      ...
      "Email": r.user_email,
      "Endereço do Missionário": r.user_address,
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-left py-2 px-2 text-muted-foreground font-medium">Endereço</th>
